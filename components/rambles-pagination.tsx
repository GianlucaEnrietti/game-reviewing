...
export default function RamblesPagination({ currentPage, totalPages }: Props) {
  if (totalPages <= 1) {
    return null;
  }
  ...
  return (
    <nav
      ...
      aria-label="Paginación de rambles"
    >
      {currentPage > 1 ? (
        <Link
          href={ramblesPageUrl(currentPage - 1)}
          ...
          return (
            <span key={page} className="flex items-center gap-4">
              ...
              {page === currentPage ? (
                ...
                <Link href={ramblesPageUrl(page)} className="hover:text-white">
                  {page}
                </Link>
              )}
            </span>
